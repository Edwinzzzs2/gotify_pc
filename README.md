...
- **消息收藏与筛选**：支持收藏重要消息，并提供仅查看收藏消息的专属视图
- **关键词搜索**：支持对消息标题和内容进行关键词实时过滤搜索
- **Bark 消息转发**：支持将特定分组的消息即时转发到 iOS Bark App，实现移动端同步推送
- **开机自动启动**：支持设置应用随系统启动（默认关闭，可在设置中开启）
- **数据安全迁移**：版本更新时默认保留用户配置与历史记录（存储在系统 AppData 目录），同时支持便携模式（数据随 EXE 移动）
- 历史消息本地存储与清空
- 系统托盘菜单与最小化到托盘
- 设置页支持查看数据存储路径（支持一键打开目录）
...
- **Bark 转发**：配置 Bark 服务器地址及需要转发的应用分组
- 启动与托盘行为（开机自启、最小化到托盘）
- 数据存储路径（查看当前路径）
...
2. **便携模式**：如果 EXE 同级目录下存在 `config.json`，优先使用该目录
3. Electron `userData` 目录（默认）：`%APPDATA%\GotifyClient`，升级软件不会丢失配置
